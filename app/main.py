...
accepted_files = ["application/pdf", "application/msword",
    ...
    for file in uploaded_files:
        if (file.content_type not in accepted_files):
            raise _fastapi.HTTPException(
                status_code=400, detail="Invalid file type. Only DOC, DOCX, and PDF files are allowed.")
        i = i + 1
...
@app.put('/api/delist')
async def delist(metadata_id: str = _fastapi.Form(), db: _orm.Session = _fastapi.Depends(_services.get_db)):
    return await _listing_services.delist(metadata_id=metadata_id, db=db)
