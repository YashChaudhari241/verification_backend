...
async def create_listing(
    ...
    stmt = update(Listings).\
        where(Listings.property_id == new_listing.property_id).\
...
async def login_user(wallet_address: str, signed_nonce: str, db: "Session") -> str:
    ...
    result = requests.post("http://"+os.environ['UTILS_HOST']+":3000/api/signature", json={
                           "nonce": nonce, "publicAddress": wallet_address, "signature": signed_nonce}).json()
...
async def get_nonce(
    ...
    result = db.query(WalletNonce). \
        ...
        one_or_none()
    if result is None:
        new_entry = WalletNonce(
            wallet_address=wallet_address, nonce=generate(size=8))
        db.add(new_entry)
        db.commit()
...
async def connect_aadhar(wallet_address: str, signed_nonce: str, aadharno: str, db: "Session") -> str:
    ...
    if "error" in otp:
        return otp
    result = requests.post("http://"+os.environ['UTILS_HOST']+":3000/api/signature", json={
                           "nonce": otp["otp"], "publicAddress": wallet_address, "signature": signed_nonce}).json()
    print(result)
    print(otp["otp"])
    if ("address" in result and result["address"] == wallet_address.lower()):
        ...
        result = db.query(AadharConnect). \
            ...
            one_or_none()
        if result is None:
            conn_obj = AadharConnect(
                wallet_address=wallet_address, UID=aadharno)
            db.add(conn_obj)
            db.commit()
        return {"access_token": jwt.encode({"wallet_address": wallet_address, "created": datetime.now().strftime("%m/%d/%Y, %H:%M:%S")}, "secret-key", algorithm="HS256")}
    else:
